...
const projectEstimate = document.querySelector('#projectEstimate');
...
document.querySelector('#addKeychain').addEventListener('click', () => {
  cart.push({ name: `Brelok ${keychainText.value || 'TWOJE IMIĘ'}`, price: 29, image: 'assets/keychain-placeholder.svg' });
  renderCart();
  openCart();
});

function updateProjectEstimate() {
  const length = Number(document.querySelector('#projectLength').value) || 0;
  const width = Number(document.querySelector('#projectWidth').value) || 0;
  const height = Number(document.querySelector('#projectHeight').value) || 0;
  const finish = Number(document.querySelector('#projectFinish').value) || 0;
  const quantity = Math.max(1, Number(document.querySelector('#projectQuantity').value) || 1);
  const sizeSurcharge = length && width && height ? Math.min(120, Math.ceil((length * width * height) / 100000)) : 0;
  const singlePrice = 50 + 25 + finish + sizeSurcharge;
  projectEstimate.textContent = money(singlePrice * quantity);
}

['#projectLength', '#projectWidth', '#projectHeight', '#projectFinish', '#projectQuantity'].forEach((selector) => {
  document.querySelector(selector).addEventListener('input', updateProjectEstimate);
  document.querySelector(selector).addEventListener('change', updateProjectEstimate);
});

projectForm.addEventListener('submit', (event) => {
  event.preventDefault();
  const length = Number(document.querySelector('#projectLength').value);
  const width = Number(document.querySelector('#projectWidth').value);
  const height = Number(document.querySelector('#projectHeight').value);
  const quantity = Math.max(1, Number(document.querySelector('#projectQuantity').value) || 1);
  const error = document.querySelector('#projectError');
  if (!length || !width || !height || length < 1 || width < 1 || height < 1) {
    error.textContent = 'Podaj długość, szerokość i wysokość w milimetrach.';
    return;
  }
  const finish = Number(document.querySelector('#projectFinish').value) || 0;
  const sizeSurcharge = Math.min(120, Math.ceil((length * width * height) / 100000));
  const unitPrice = 50 + 25 + finish + sizeSurcharge;
  const filament = document.querySelector('#projectFilament').value;
  const notes = document.querySelector('#projectNotes').value.trim();
  cart.push({
    name: `Projekt indywidualny (${length} × ${width} × ${height} mm) × ${quantity}`,
    price: unitPrice * quantity,
    image: 'assets/keychain-placeholder.svg',
    details: `${filament}; ${notes || 'bez dodatkowych uwag'}`
  });
  error.textContent = '';
  renderCart();
  openCart();
...
document.querySelector('#toPayment').addEventListener('click', () => {
  ...
  const address = delivery && delivery.value === 'Paczkomat' ? document.querySelector('#lockerCode').value.trim() : document.querySelector('#shippingAddress').value.trim();
...
document.querySelector('#placeOrder').addEventListener('click', () => {
  ...
  const order = {
    ...
    customer: {
      ...
      email: document.querySelector('#shippingEmail').value.trim()
    },
    delivery: {
      carrier: carrier ? carrier.dataset.carrier : '',
      type: delivery ? delivery.value : '',
      address: delivery && delivery.value === 'Paczkomat' ? document.querySelector('#lockerCode').value.trim() : document.querySelector('#shippingAddress').value.trim()
    },
    items: cart.map((item) => ({ name: item.name, price: item.price, details: item.details || '' })),
    total: cart.reduce((sum, item) => sum + item.price, 0),
    payment: 'BLIK'
  };
  const savedOrders = JSON.parse(localStorage.getItem('kreslOrders') || '[]');
  savedOrders.push(order);
  localStorage.setItem('kreslOrders', JSON.stringify(savedOrders));
